...
"""ResNet and DenseNet in PyTorch.
ResNet
...
DenseNet
[2] Iandola F, Moskewicz M, Karayev S, et al.
    Densenet: Implementing efficient convnet descriptor pyramids. arXiv:1404.1869
...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        building_blocks.append(first)
        block_idx, self.block_indices = 0, []
        ...
        for i, num_layers in enumerate(block_config):
            ...
            if i != len(block_config) - 1:
                ...
                building_blocks.append(trans)
                block_idx += 1
                self.block_indices.append(block_idx)
                num_features = num_features // 2

        # Final batch norm
        building_blocks.append(nn.BatchNorm2d(num_features))
        self.model = nn.Sequential(*building_blocks)
    ...
    def forward(self, x):
        features = []
        for idx, building_block in enumerate(self.model):
            x = building_block(x)
            if idx in self.block_indices:
                features.append(F.adaptive_avg_pool2d(x, 1).flatten(1, 3))
